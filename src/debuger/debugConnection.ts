...
export class WebsocketConnection implements IDebugConnection {
  ...
  connect(): void {
    this.websocket = new Promise((resolve, reject) => {
      const ws: WebSocket = new WebSocket(this.address);
  ...
  private handleMessage(ev: MessageEvent) {
    if (
      (this.messageCallback !== null && this.messageCallback.allowListeners) ||
      ...
      if (this.messageCallback.waitUntil) {
        ...
        protocolMessages.forEach((message) => {
          ...
          if (
            ...
            this.messageCallback?.waitUntil?.includes(
              (message as DebugProtocol.Response).command
            )
          ) {
            this.messageCallback.waitUntil =
              this.messageCallback.waitUntil.filter(
                (command) =>
                  command !== (message as DebugProtocol.Response).command
              );
          }
          if (
            message.type == "event" &&
            this.messageCallback?.waitUntil?.includes(
              (message as DebugProtocol.Event).event
            )
          ) {
