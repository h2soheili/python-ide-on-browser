...
export class MessageUtil {
  ...
  // handles encoded messages and triggers the right events based on the message content
      ...
      switch (protocolMessage.type) {
        case "response":
          ...
          if (response.success === false) {
            console.error(
              "Debugger has rejected the request, beacuse " + response.message,
              response
            );
          } else {
        ...
        default:
          console.log(
            "Unsupported Messagetype: " + protocolMessage.type,
            protocolMessage
          );
          break;
      }
