...
class Styles {
  static disabled: IStyle = {
    filter: "grayscale(100%) brightness(80%); cursor: inherit",
  };
...
export class Renderer {
  ...
  updateToolbox(action: keyof IButtonEvents, object?: HTMLElement) {
    switch (action) {
      case "start":
        const continu = this.renderButton(
          Media.continueSvg,
          "continue",
          "toolbox button"
        );
        if (this.toolbox.firstChild)
          this.toolbox.replaceChild(continu, this.toolbox.firstChild);
  ...
  renderStopLine(line: number) {
    for (let i = 1; i <= (this.editor.getModel()?.getLineCount() || 1); i++) {
      ...
      this.editor.deltaDecorations(
        currentDecorations
          ?.filter((value) => value.options.className === "myContentClass")
          .map((value) => value.id) || [],
        []
      );
    }
    if (line !== -1) {
      ...
      if (
        currentDecorations?.filter(
          (value) => value.options.className === "myContentClass"
        ).length === 0
      ) {
  ...
  renderVariablesToDrawer(
    drawerName: string,
    varObject: DebugProtocol.Variable[]
  ): void {
    this.setDrawerContent(drawerName, this.renderVariables(varObject));
  }

  renderVariablesToElement(
    element: HTMLElement,
    varObject: DebugProtocol.Variable[]
  ): void {
    ...
    if (elem.firstChild) {
      const arrow = this.renderElement(
        "span",
        "",
        Media.downArrow.replace("c-2", "drawer variables arrow")
      );
      elem.replaceChild(arrow, elem.firstChild);
      arrow.addEventListener("click", () =>
        this.events.process("button", "varClose", elem)
      );
    }
  }
    ...
    if (element) {
      ...
      if (elem.firstChild) {
        const arrow = this.renderElement(
          "span",
          "",
          Media.rightArrow.replace("c-1", "drawer variables arrow")
        );
        elem.replaceChild(arrow, elem.firstChild);
        arrow.addEventListener("click", () =>
          this.events.process(
            ...
            "varOpen",
            elem,
            Number(elem.getAttribute("varRef"))
          )
        );
      }
    }
  ...
  renderVariables(varObject: DebugProtocol.Variable[]): HTMLElement {
    ...
    varObject.forEach((variable) => {
      ...
      switch (variable.type) {
        ...
        case "int":
          listentry.appendChild(
            this.renderElement("span", "drawer variables int", variable.value)
          );
          break;
        default:
          listentry.appendChild(
  ...
  renderStackFramesToDrawer(
    drawerName: string,
    varObject: DebugProtocol.StackFrame[]
  ): void {
  ...
  renderToolbox() {
    ...
    wrapper.appendChild(
      this.renderButton(Media.stepOverSvg, "stepOver", "toolbox button", true)
    );
    wrapper.appendChild(
      this.renderButton(Media.restartSvg, "restart", "toolbox button", true)
    );
    wrapper.appendChild(
      this.renderButton(Media.stopSvg, "stop", "toolbox button", true)
    );
    return wrapper;
  }
  ...
  renderDrawer(name: string, content?: HTMLElement) {
    ...
    const contentWrapper = this.renderWrapper(
      "drawer body drawer-content noselect"
    );
    if (content) {
  ...
  renderButton(
    ...
    button.addEventListener("click", () =>
      this.events.process("button", action, button)
    );
    return button;
  }
  ...
  processClick(
    ...
    this.buttonEventListener[action]?.forEach((listener) =>
      listener(object, data)
    );
  }

  on(
    event: keyof IButtonEvents,
  ...
  resizeHorizontal(event: MouseEvent, element: HTMLElement) {
    ...
    this.events.process("button", "resize", this.domElement, {
      width: box.width,
      height: box.height,
    });
  }
    ...
    listen.addEventListener("mousedown", () => {
      if (direction === "h") {
        document.onmousemove = (event) =>
          this.resizeHorizontal.call(this, event, resize);
      }
      if (direction === "v") {
        document.onmousemove = (event) =>
          this.resizeVertical.call(this, event, resize);
      }
      document.onmouseup = () => {
