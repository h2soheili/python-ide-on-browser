...
export { ThemeVSLight, ThemeVSDark };
export type { IDebuggerTheme };
export { WebsocketConnection };
export type { IDebugConnection };
...
export default class Debugger {
  ...
  constructor(
    ...
    if (
      ...
      typeof options.shortcuts !== "boolean"
    ) {
    ...
    this.events.on("button", "stepOver", () => {
      this.connection?.sendMessage(
        this.protocolProvider.next(this.threads.getCurrentThreadId())
      );
    });
        ...
        this.connection?.sendMessage(
          ...
          (message) => {
            this.messageUtil
              ...
              .forEach((vars) =>
                this.renderer.renderVariablesToElement(
                  object,
                  vars.body.variables
                )
              );
    ...
    this.events.on("button", "varClose", (elem) =>
      this.renderer.removeVariablesFromElement(elem)
    );
    ...
    this.events.on("button", "continue", () => {
      this.connection?.sendMessage(
        this.protocolProvider.continue(this.threads.getCurrentThreadId())
      );
    });
    ...
    this.events.on("response", "initialize", () => {
      this.connection?.sendMessage(
        this.protocolProvider.launch(this.debugArguments)
      );
    ...
    this.events.on("event", "initialized", () => {
      if (this.breakpoints.breakpoints.length === 0) {
        ...
        this.connection?.sendMessage(
          this.protocolProvider.setBreakpoints(this.breakpoints.breakpoints)
        );
      }
    });
    // Need to check if breakpoints are available, else start program without breakpoints.
    ...
    this.events.on("event", "stopped", () => {
      this.connection?.sendMessage(
        this.protocolProvider.stackTrace(this.threads.getCurrentThreadId())
      );
    });
    this.events.on("response", "stackTrace", (data) => {
      ...
      this.renderer.renderStackFramesToDrawer(
        "<b>CALL STACK</b>",
        stackTrace.body.stackFrames
      );
    ...
    this.events.on("response", "scopes", (data) => {
      ...
      this.connection?.sendMessage(
        this.protocolProvider.variables(
    ...
    this.events.on("response", "variables", (data) => {
      ...
      this.renderer.renderVariablesToDrawer(
        "<b>VARIABLES</b>",
  ...
  public attachDebugAdapterConnection(connection: IDebugConnection) {
    ...
    this.connection?.on(
      "message",
      this.messageUtil.handleMessageFromString.bind(this.messageUtil)
    );
  }
  ...
  public on(
    type: "button",
    listener: (element?: HTMLElement, data?: any) => {}
  ): void;
  public on(
    type: "resize",
    listener: (element?: HTMLElement, data?: any) => {}
